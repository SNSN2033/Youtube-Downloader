...
app = Flask(__name__)
app.secret_key = "shining@test.com"
...
def login_method():
    if request.method == 'POST':
        ...
        check = User.isLoginValid(account, password)
        if check:
            session['account'] = account
            session['name'] = User.findUserData(account).get('name')
            return redirect("/")
        else:
